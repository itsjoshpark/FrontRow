...
extension ConversionSuites {
    ...
    struct ConversionResourceTests {
        ...
        func cancellingLeavesNoProcessRunning() async throws {
            ...
            #expect(
                await waitUntilRunning(tool),
                "The tool never started, so there was nothing to cancel")

            task.cancel()
            await #expect(throws: FFmpegError.cancelled) { try await task.value }

            #expect(
                !isRunning(tool), "The tool was still running after the conversion was cancelled")
        }
        ...
        func repeatedConversionsGiveTheirPipesBack() async throws {
            ...
            try await remuxer.remux(input: input, output: output, recipe: recipe, duration: 10) {
                _ in
            }
